...
class cst:
    ...
    def p1(self, x):
        ...
        return antoine(x[self.index], self.param[0], self.param[1], self.param[2])
    def p2(self, x):
        # return antoine(self.T, x[3], x[4], x[5])
        return antoine(x[self.index], self.param[3], self.param[4], self.param[5])
    def gamma1(self, x):
        # return nrtl_binary(self.z1_mol, self.T, R, x[6], x[7], x[8])[0] # nrtl_binary中には既に計算されているため、+273.15を削除
        return nrtl_binary(self.z1_mol, x[self.index], R, self.param[6], self.param[7], self.param[8])[0] # nrtl_binary中には既に計算されているため、+273.15を削除
    def gamma2(self, x):
        # return nrtl_binary(self.z1_mol, self.T, R, x[6], x[7], x[8])[1] # nrtl_binary中には既に計算されているため、+273.15を削除
        return nrtl_binary(self.z1_mol, x[self.index], R, self.param[6], self.param[7], self.param[8])[1] # nrtl_binary中には既に計算されているため、+273.15を削除
    def x1(self, x):
...
def get_each_yield_cal(z1_mol, z1, p_total, M1, M2, F_in, S, T):
    ...
    # d_g12 = -4949.805319
    # d_g21 = -4678.417759
    # param = [A1, B1, C1, A2, B2, C2, alpha, d_g12, d_g21]
    # return np.array([yield_cal(z1_mol[i], z1[i], p_total[i], M1, M2, F_in[i], param[0], param[1], param[2], param[3], param[4], param[5], param[6], param[7], param[8], T[i]) for i in range(len(z1_mol))])
...
def get_each_yield_cal_all(z1_mol, z1, p_total, M1, M2, F_in, S, T):
    ...
    # d_g21 = -4678.417759
    # param = [A1, B1, C1, A2, B2, C2, alpha, d_g12, d_g21]
    # ret = np.array([yield_cal(z1_mol[i], z1[i], p_total[i], M1, M2, F_in[i], param[0], param[1], param[2], param[3], param[4], param[5], param[6], param[7], param[8], T[i], return_vars=True) for i in range(len(z1_mol))])
